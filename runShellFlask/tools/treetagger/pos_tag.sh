...
rm -f tmp/${TIMESTAMP}split_files*
...
if [ $num_lines -le 0 ] || [ $THREAD -eq 1 ]; then
	cp $RAW tmp/${TIMESTAMP}split_files.00000
else
	csplit -s -f "tmp/${TIMESTAMP}split_files." -n 5  $RAW $num_lines {$(($THREAD-2))}
fi
for f in tmp/${TIMESTAMP}split_files.*
do
    sed -e 's/ /\'$'\n/g' $f > $f.token &
done
wait
...
cd tools/treetagger/
for f in ../../tmp/${TIMESTAMP}split_files.*.token
do
    if [ $LANGUAGE == "CN" ]; then
...
cd ../../
rm -f ./tmp/${TIMESTAMP}pos_tags.txt
echo -ne "Current step: Merging...\033[0K\r"
for f in tmp/${TIMESTAMP}split_files.*.tagged
do
    cat $f >> ./tmp/${TIMESTAMP}pos_tags.txt
done
echo
